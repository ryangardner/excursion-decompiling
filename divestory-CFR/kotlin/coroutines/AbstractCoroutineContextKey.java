...
implements CoroutineContext.Key<E> {
    ...
    public AbstractCoroutineContextKey(CoroutineContext.Key<B> key, Function1<? super CoroutineContext.Element, ? extends E> key2) {
        Intrinsics.checkParameterIsNotNull(key, "baseKey");
        Intrinsics.checkParameterIsNotNull(key2, "safeCast");
        this.safeCast = key2;
        key2 = key;
        if (key instanceof AbstractCoroutineContextKey) {
            key2 = ((AbstractCoroutineContextKey)key).topmostKey;
        }
        this.topmostKey = key2;
    }

    public final boolean isSubKey$kotlin_stdlib(CoroutineContext.Key<?> key) {
        Intrinsics.checkParameterIsNotNull(key, "key");
        if (key == this) return true;
        if (this.topmostKey == key) return true;
        return false;
    }
