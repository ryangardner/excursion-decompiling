...
public class TypefaceCompat {
    ...
    public static Typeface createFromResourcesFamilyXml(Context object, FontResourcesParserCompat.FamilyResourceEntry familyResourceEntry, Resources resources, int n, int n2, ResourcesCompat.FontCallback fontCallback, Handler handler, boolean bl) {
        if (familyResourceEntry instanceof FontResourcesParserCompat.ProviderResourceEntry) {
            ...
            boolean bl2 = false;
            if (bl ? ((FontResourcesParserCompat.ProviderResourceEntry)familyResourceEntry).getFetchStrategy() == 0 : fontCallback == null) {
                bl2 = true;
            }
            int n3 = bl ? ((FontResourcesParserCompat.ProviderResourceEntry)familyResourceEntry).getTimeout() : -1;
            object = FontsContractCompat.getFontSync(object, ((FontResourcesParserCompat.ProviderResourceEntry)familyResourceEntry).getRequest(), fontCallback, handler, bl2, n3, n2);
        } else {
    ...
    public static Typeface createFromResourcesFontFile(Context context, Resources object, int n, String string2, int n2) {
        if ((context = sTypefaceCompatImpl.createFromResourcesFontFile(context, (Resources)object, n, string2, n2)) == null) return context;
    ...
    private static Typeface getBestFontFromFamily(Context context, Typeface object, int n) {
        if ((object = sTypefaceCompatImpl.getFontFamily((Typeface)object)) != null) return sTypefaceCompatImpl.createFromFontFamilyFilesResourceEntry(context, (FontResourcesParserCompat.FontFamilyFilesResourceEntry)object, context.getResources(), n);
        return null;
    }
}
