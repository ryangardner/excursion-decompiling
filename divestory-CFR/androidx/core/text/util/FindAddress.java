...
class FindAddress {
    ...
    private static boolean isValidZipCode(String string2, MatchResult matchResult) {
        ...
        boolean bl2 = bl;
        if (!sZipCodeRe.matcher(string2).matches()) return bl2;
        bl2 = bl;
        if (!sStateZipCodeRanges[n].matches(string2)) return bl2;
        return true;
    }
    ...
    private static class ZipRange {
        ...
        boolean matches(String string2) {
            ...
            if (this.mLow <= n) {
                if (n <= this.mHigh) return true;
            }
            if (n == this.mException1) return true;
