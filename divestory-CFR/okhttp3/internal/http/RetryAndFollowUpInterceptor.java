...
implements Interceptor {
    ...
    private final Request buildRedirectRequest(Response response, String string2) {
        ...
        if (!Intrinsics.areEqual(httpUrl.scheme(), response.request().url().scheme()) && !this.client.followSslRedirects()) {
            return null;
        }
        object = response.request().newBuilder();
        if (HttpMethod.permitsRequestBody(string2)) {
            int n = response.code();
            boolean bl2 = HttpMethod.INSTANCE.redirectsWithBody(string2) || n == 308 || n == 307;
            if (HttpMethod.INSTANCE.redirectsToGet(string2) && n != 308 && n != 307) {
                ((Request.Builder)object).method("GET", null);
            } else {
                if (bl2) {
                    requestBody = response.request().body();
                }
                ((Request.Builder)object).method(string2, requestBody);
            }
            if (!bl2) {
        ...
        if (Util.canReuseConnectionFor(response.request().url(), httpUrl)) return ((Request.Builder)object).url(httpUrl).build();
        ((Request.Builder)object).removeHeader("Authorization");
        return ((Request.Builder)object).url(httpUrl).build();
    }

    /*
     * Exception decompiling
     */
    private final Request followUpRequest(Response var1_1, Exchange var2_2) throws IOException {
    ...
    private final boolean requestIsOneShot(IOException iOException, Request object) {
        if ((object = ((Request)object).body()) != null) {
            if (((RequestBody)object).isOneShot()) return true;
        }
        if (iOException instanceof FileNotFoundException) return true;
        return false;
    }

    private final int retryAfter(Response object, int n) {
        String string2 = Response.header$default((Response)object, "Retry-After", null, 2, null);
        ...
        Intrinsics.checkExpressionValueIsNotNull(object, "Integer.valueOf(header)");
