...
public class ElevationOverlayProvider {
    ...
    private boolean isThemeSurfaceColor(int n) {
        if (ColorUtils.setAlphaComponent(n, 255) != this.colorSurface) return false;
        return true;
    }
    ...
    public float calculateOverlayAlphaFraction(float f) {
        ...
        if (!(f <= 0.0f)) return Math.min(((float)Math.log1p(f / f2) * 4.5f + 2.0f) / 100.0f, 1.0f);
        return 0.0f;
    }
    ...
    public int compositeOverlayIfNeeded(int n, float f) {
        int n2 = n;
        if (!this.elevationOverlayEnabled) return n2;
        n2 = n;
        if (!this.isThemeSurfaceColor(n)) return n2;
