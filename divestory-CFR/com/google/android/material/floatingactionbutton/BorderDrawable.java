...
extends Drawable {
    ...
    public void draw(Canvas canvas) {
        ...
        if (!this.shapeAppearanceModel.isRoundRect(this.getBoundsAsRectF())) return;
    ...
    public int getOpacity() {
        if (!(this.borderWidth > 0.0f)) return -2;
        return -3;
    }
    ...
    public void getOutline(Outline outline) {
        ...
        if (!this.shapePath.isConvex()) return;
        outline.setConvexPath(this.shapePath);
    }

    public boolean getPadding(Rect rect) {
        if (!this.shapeAppearanceModel.isRoundRect(this.getBoundsAsRectF())) return true;
