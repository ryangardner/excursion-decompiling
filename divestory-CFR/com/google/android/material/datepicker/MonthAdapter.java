...
extends BaseAdapter {
    ...
    private void initializeStyles(Context context) {
        if (this.calendarStyle != null) return;
    ...
    public Long getItem(int n) {
        if (n < this.month.daysFromStartOfWeekToFirstOfMonth()) return null;
        if (n <= this.lastPositionInMonth()) return this.month.getDay(this.positionToDay(n));
        return null;
    }
    ...
    public TextView getView(int n, View object, ViewGroup object2) {
        ...
        if (!this.calendarConstraints.getDateValidator().isValid((Long)object)) {
            textView.setEnabled(false);
        ...
        while (object2.hasNext()) {
            l = (Long)object2.next();
            if (UtcDates.canonicalYearMonthDay((Long)object) != UtcDates.canonicalYearMonthDay(l)) continue;
            this.calendarStyle.selectedDay.styleItem(textView);
            return textView;
        }
        if (UtcDates.getTodayCalendar().getTimeInMillis() == ((Long)object).longValue()) {
    ...
    boolean isFirstInRow(int n) {
        if (n % this.month.daysInWeek != 0) return false;
        return true;
    }

    boolean isLastInRow(int n) {
        boolean bl = true;
        if ((n + 1) % this.month.daysInWeek != 0) return false;
        return bl;
    }
    ...
    boolean withinMonth(int n) {
        if (n < this.firstPositionInMonth()) return false;
        if (n > this.lastPositionInMonth()) return false;
        return true;
    }
}
