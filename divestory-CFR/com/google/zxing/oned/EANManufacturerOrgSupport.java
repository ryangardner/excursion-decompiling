...
final class EANManufacturerOrgSupport {
    ...
    private void add(int[] arrn, String string2) {
        this.ranges.add(arrn);
        this.countryIdentifiers.add(string2);
    }

    private void initIfNeeded() {
        synchronized (this) {
            boolean bl = this.ranges.isEmpty();
            if (!bl) {
                return;
            }
    ...
    String lookupCountryIdentifier(String arrn) {
        this.initIfNeeded();
        int n = Integer.parseInt(arrn.substring(0, 3));
        int n2 = this.ranges.size();
        int n3 = 0;
        while (n3 < n2) {
            arrn = this.ranges.get(n3);
            int n4 = arrn[0];
            if (n < n4) {
                return null;
            }
            ...
            if (n <= n4) {
                return this.countryIdentifiers.get(n3);
            }
            ++n3;
        }
        return null;
