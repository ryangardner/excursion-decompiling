...
final class BitMatrixParser {
    ...
    byte[] readCodewords() {
        byte[] arrby = new byte[144];
        int n = this.bitMatrix.getHeight();
        int n2 = this.bitMatrix.getWidth();
        int n3 = 0;
        while (n3 < n) {
            int[] arrn = BITNR[n3];
            for (int i = 0; i < n2; ++i) {
                int n4 = arrn[i];
                if (n4 < 0 || !this.bitMatrix.get(i, n3)) continue;
                int n5 = n4 / 6;
                byte by = arrby[n5];
                arrby[n5] = (byte)((byte)(1 << 5 - n4 % 6) | by);
            }
            ++n3;
        }
        return arrby;
