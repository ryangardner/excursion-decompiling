...
public class ElevationOverlayProvider {
   ...
   public ElevationOverlayProvider(Context var1) {
      this.elevationOverlayEnabled = MaterialAttributes.resolveBoolean(var1, R.attr.elevationOverlayEnabled, false);
      this.elevationOverlayColor = MaterialColors.getColor((Context)var1, R.attr.elevationOverlayColor, 0);
      this.colorSurface = MaterialColors.getColor((Context)var1, R.attr.colorSurface, 0);
      this.displayDensity = var1.getResources().getDisplayMetrics().density;
   }

   private boolean isThemeSurfaceColor(int var1) {
      boolean var2;
      if (ColorUtils.setAlphaComponent(var1, 255) == this.colorSurface) {
         var2 = true;
      } else {
         var2 = false;
      }
   ...
   public int compositeOverlayIfNeeded(int var1, float var2) {
      ...
      if (this.elevationOverlayEnabled) {
         var3 = var1;
         if (this.isThemeSurfaceColor(var1)) {
            var3 = this.compositeOverlay(var1, var2);
         }
      }
