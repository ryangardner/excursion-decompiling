...
public class CircularProgressDrawable extends Drawable implements Animatable {
   ...
   private int evaluateColorChange(float var1, int var2, int var3) {
      ...
      int var6 = var2 >> 8 & 255;
      var2 &= 255;
      return var4 + (int)((float)((var3 >> 24 & 255) - var4) * var1) << 24 | var5 + (int)((float)((var3 >> 16 & 255) - var5) * var1) << 16 | var6 + (int)((float)((var3 >> 8 & 255) - var6) * var1) << 8 | var2 + (int)(var1 * (float)((var3 & 255) - var2));
   }
   ...
   private void setSizeParameters(float var1, float var2, float var3, float var4) {
      CircularProgressDrawable.Ring var5 = this.mRing;
      float var6 = this.mResources.getDisplayMetrics().density;
      var5.setStrokeWidth(var2 * var6);
      var5.setCenterRadius(var1 * var6);
      var5.setColorIndex(0);
      var5.setArrowDimensions(var3 * var6, var4 * var6);
   }
   ...
   void applyTransformation(float var1, CircularProgressDrawable.Ring var2, boolean var3) {
      if (this.mFinishing) {
         ...
         float var4 = var2.getStartingRotation();
         float var5;
         float var6;
         if (var1 < 0.5F) {
            var5 = var1 / 0.5F;
            var6 = var2.getStartingStartTrim();
            var5 = MATERIAL_INTERPOLATOR.getInterpolation(var5) * 0.79F + 0.01F + var6;
         } else {
            var6 = (var1 - 0.5F) / 0.5F;
            var5 = var2.getStartingStartTrim() + 0.79F;
            var6 = var5 - ((1.0F - MATERIAL_INTERPOLATOR.getInterpolation(var6)) * 0.79F + 0.01F);
         }

         float var7 = this.mRotationCount;
         var2.setStartTrim(var6);
         var2.setEndTrim(var5);
         var2.setRotation(var4 + 0.20999998F * var1);
   ...
   private static class Ring {
      ...
      void draw(Canvas var1, Rect var2) {
         ...
         float var5 = this.mStrokeWidth / 2.0F + var4;
         if (var4 <= 0.0F) {
            var5 = (float)Math.min(var2.width(), var2.height()) / 2.0F - Math.max((float)this.mArrowWidth * this.mArrowScale / 2.0F, this.mStrokeWidth / 2.0F);
         }

         var3.set((float)var2.centerX() - var5, (float)var2.centerY() - var5, (float)var2.centerX() + var5, (float)var2.centerY() + var5);
         var5 = this.mStartTrim;
         var4 = this.mRotation;
         var5 = (var5 + var4) * 360.0F;
         var4 = (this.mEndTrim + var4) * 360.0F - var5;
         this.mPaint.setColor(this.mCurrentColor);
         this.mPaint.setAlpha(this.mAlpha);
         float var6 = this.mStrokeWidth / 2.0F;
         var3.inset(var6, var6);
         var1.drawCircle(var3.centerX(), var3.centerY(), var3.width() / 2.0F, this.mCirclePaint);
         var6 = -var6;
         var3.inset(var6, var6);
         var1.drawArc(var3, var5, var4, false, this.mPaint);
         this.drawTriangle(var1, var5, var4, var3);
      }

      void drawTriangle(Canvas var1, float var2, float var3, RectF var4) {
         if (this.mShowArrow) {
            Path var5 = this.mArrow;
            if (var5 == null) {
               var5 = new Path();
               this.mArrow = var5;
               var5.setFillType(FillType.EVEN_ODD);
            } else {
               var5.reset();
            }

            float var6 = Math.min(var4.width(), var4.height()) / 2.0F;
            float var7 = (float)this.mArrowWidth * this.mArrowScale / 2.0F;
            this.mArrow.moveTo(0.0F, 0.0F);
            this.mArrow.lineTo((float)this.mArrowWidth * this.mArrowScale, 0.0F);
            var5 = this.mArrow;
            float var8 = (float)this.mArrowWidth;
            float var9 = this.mArrowScale;
            var5.lineTo(var8 * var9 / 2.0F, (float)this.mArrowHeight * var9);
            this.mArrow.offset(var6 + var4.centerX() - var7, var4.centerY() + this.mStrokeWidth / 2.0F);
            this.mArrow.close();
